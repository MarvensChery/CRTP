...
router.post('/', async (req, res) => {
    if (req.body.Identifiant === undefined || req.body.MotDePasse === undefined) return res.status(400).json({ message: 'Le courriel out le mot de passe ne peut pas être vide' });

    try {
        const resultat = await request.getUtilisateurByIdentifiant(req.body.Identifiant);
